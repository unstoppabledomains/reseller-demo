...
const Send = (props) => {
	...
	const _renderBack = () => {
		return <button className="btn btn-md btn-primary" onClick={(e) => props.history.push('/')}>Back</button>
	}
		...
		if (_checkForErrors(domain)) {
			...
			if (resolution.meta.owner) {
				...
				if (canBuy && canBuy.domain && canBuy.domain.reselling)
					...
					setCanResell(false);
				if (canBuy && canBuy.errors)
					setResellErrors(canBuy.errors);
				else
					setResellErrors(null);
	...
	const _renderHint = () => {
		...
		if (resolvedAddress === 'NOT_FOUND')
			return <small className="text-danger">No {selectedTicker} address is registered under this domain</small>
		if (resolvedAddress === 'NO_OWNER')
			return (
				<>
					{resellErrors ? resellErrors.map((error, index) => <small key={index} className="text-danger">{error.message}</small>) :
						<small className="text-warning">Domain is not taken
				{canResell ?
								<Link to={
									{
										pathname: '/email',
		...
		if (resolvedAddress === 'INVALID_DOMAIN')
			return <small className="text-danger">Domain is invalid or unavailable</small>
		return <small className="text-success">Resolved Address: {resolvedAddress}</small>;
	}
							...
							<form onSubmit={_handleSubmit}>
								...
								<div className="form-group">
									<div className="input-group mb-3">
										...
										<input type="number" className="form-control" id="reciever" placeholder="Amount" />
									</div>
								</div>
								<div className="form-group">
									<textarea rows="6" className="form-control" id="reciever" placeholder="Message (Optional)" />
								</div>
								<Link to="/">
